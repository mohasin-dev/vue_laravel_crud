            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <div class="card-tools" style="position:absolute; right:2rem; top:1rem;">
                            <button @click="reload" class="btn btn-primary btn-sm" type="button">
                                Reload
                            </button>
                            <button @click="create" class="btn btn-primary btn-sm" type="button">
                                Create New
                            </button>
                        </div>
                    </div>
                                ...
                                <tbody>
                                    <tr v-show="customers.length" v-for="(customer, index) in customers" :key="customer.id">
                                        ...
                                        <td class="text-center">
                                            ...
                                            <button type="button" @click="edit(customer)" class="btn btn-success btn-sm">
                                    ...
                                    <tr v-show="!customers.length">
                                        <td colspan="6">
                                            <div class="alert alert-danger" role="alert">
                                                <h5 class="text-center pt-3">Sorry ! No data Found................ &nbsp;&nbsp;:(</h5>
                                            </div>
                                        </td>
                                    </tr>
                                </tbody>
...
<script>
    export default {
        ...
        methods:{
            getData(){
                this.$Progress.start()
                axios.get('/laravel_vue_api_crud/public/api/customers?page='+this.pagination.current_page)
                    .then(response =>{
            ...
            searchData(){
                this.$Progress.start()
                axios.get('/laravel_vue_api_crud/public/api/search/customers/'+this.queryFiled+'/'+this.query+'?page='+this.pagination.current_page)
                    .then(response =>{
                        this.customers = response.data.data
                        this.pagination = response.data.meta
                        this.$Progress.finish()
            ...
            store() {
                this.$Progress.start()
                this.form.busy = true
                this.form.post('/laravel_vue_api_crud/public/api/customers')
                    .then(respone =>{
                        this.getData()
            ...
            update(){
                this.$Progress.start()
                this.form.busy = true
                this.form.put('/laravel_vue_api_crud/public/api/customers/'+ this.form.id)
                    .then(respone =>{
                        this.getData()
                        $('#customerModalLong').modal('hide');
                         if (this.form.successful) {
                ...
                this.$snotify.confirm(
                    ...
                    buttons: [
                        ...
                        action: toast => {
                            ...
                            this.$Progress.start();
                            axios.delete("/laravel_vue_api_crud/public/api/customers/" + customer.id)
                            .then(response => {
                                this.getData();
                                this.$Progress.finish();
                                this.$snotify.success(
                                "Customer Successfully Deleted","Success");
                            })
                            .catch(e => {
                                this.$Progress.fail();
                                console.log(e);
